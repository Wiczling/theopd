...
Theoph data is available in R. Several columns needs to be added (AMT, CMT, MDV) for NONMEM run.
...
write.csv(dataall, paste0(model_dir, "/nonmemdata.csv"), row.names=FALSE)
```
...
out <- mrgsim(
  ...
  obsonly   = TRUE,
  Req       = "MRG = IPRED", 
  carry_out = "NM = IPRED, PRED, CONC") %>%
  as.data.frame() %>%
  ggplot()+
  geom_point(aes(x=TIME,y=CONC))+
  geom_line(aes(x=TIME,y=PRED), linetype=2)+
  geom_line(aes(x=TIME,y=MRG))+
  geom_line(aes(x=TIME,y=NM),color="blue")+
  labs( x="Time (h)", y="Theophylline Concentration (mg/L)")+
  facet_wrap(.~ID)
...
dose_rec <- filter(join_df, EVID==1) %>%
  ...
  mutate(II = 24, SS = 1, TIME = 0, EVID = 1, CMT = 1)
...
pmtab$data %>%
  kableExtra::kable() %>%
  kableExtra::save_kable(paste0(here::here("deliv/tables"),"/auccmaxcmin", ".png"))

pmtab$data %>% kableExtra::kable()
